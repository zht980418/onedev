...
//		demand 3 modify the fork parent
		String userName = SecurityUtils.getUser().getName()+"/personal-project";
...
//			demand3 make every user can create personal project under the root
//		} else if (SecurityUtils.canCreateRootProjects()) {
		...
		Collection<String> properties = Sets.newHashSet(PROP_NAME, PROP_DESCRIPTION, PROP_ISSUE_MANAGEMENT, PROP_FORK_PERMISSION);
...
//						demand2 judge the fork permission
						if(!getProject().isForkPermission()){
							editor.error(new Path(new PathNode.Named("name")),"you have no permission to fork this project");
							target.add(form);
							System.out.println("no permission to fork");
